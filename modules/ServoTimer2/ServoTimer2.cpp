...
ISR (TIMER2_OVF_vect) {
    ++ISRCount;  // increment the overflow counter
    if (ISRCount == servos[Channel].counter)  // are we on the final iteration for this channel
    {
        ...
        if ((Channel != FRAME_SYNC_INDEX) &&
            ...
            if (servos[Channel].Pin.isActive == true)  // check if activated
                digitalWrite(servos[Channel].Pin.nbr, HIGH);  // it's an active channel so pulse it high
...
int ServoTimer2::read() const {
    ...
    if (this->chanIndex > 0)
        ...
        pulsewidth = 0;
    return static_cast<int>(pulsewidth);
}

[[maybe_unused]] boolean ServoTimer2::attached() const {
...
static void initISR() {
    ...
    TCCR2B = _BV(CS21);  // set pre-scaler of 8
